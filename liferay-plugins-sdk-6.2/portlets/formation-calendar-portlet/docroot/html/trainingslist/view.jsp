...
<c:forEach items="${requestScope.bookings}" var="booking">
	...
	<c:otherwise>
	<c:set var="date"><fmt:formatDate value="${start }" pattern="d" /> <liferay-ui:message key="${au}" /> <fmt:formatDate value="${end }" dateStyle="long" /></c:set>
	</c:otherwise>
	</c:choose>
	<li>${date} : ${title} - Paris <button class="btn btn-mini to-form" data-training="${title}" data-start="${booking.startTime}" data-end="${booking.endTime}" data-form="${form} }"><liferay-ui:message key="${register}" /></button></li>
</c:forEach>
</ul>
